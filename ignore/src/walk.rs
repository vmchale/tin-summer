...
impl Walk {
    ...
    fn skip_entry(&self, ent: &walkdir::DirEntry) -> bool {
        ...
        let should_skip_path = skip_path(ent.path(), is_dir);
    ...
    fn run(mut self) {
        while let Some(mut work) = self.get_work() {
            ...
            for result in readdir {
                if self.run_one(depth + 1, result).is_quit() {
                    self.quit_now();
                    return;
                }
            }
    ...
    fn run_one(
        ...
        if self.follow_links && is_symlink {
            ...
            if dent.file_type().map_or(false, |ft| ft.is_dir()) {
                if let Err(err) = check_symlink_loop(dent.path(), depth) {
                    return (self.f)(Err(err));
                }
            }
        }
        ...
        let max_size = self.max_filesize;
        let should_skip_path = skip_path(dent.path(), is_dir);
        let should_skip_filesize = if !is_dir && max_size.is_some() {
...
fn skip_path(_: &Path, _: bool) -> bool {
    false
}
