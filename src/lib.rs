...
pub fn read_files(in_paths: &PathBuf, depth: u8, min_bytes: Option<u64>, regex: Option<&Regex>, silent: bool) -> FileTree {
    let mut tree = FileTree::new();
    let mut total_size = FileSize::new(0);
    ...
    if let Ok(paths) = fs::read_dir(&in_paths) {
        for p in paths {
            ...
            let loop_bool = match regex {
                Some(r) => !r.is_match(path_string),
                _ => true
            };
            if loop_bool {
                // if this fails, it's probably because `path` is a symlink, so we ignore it.
                if let Ok(metadata) = fs::metadata(&path) {
                    ...
                    // otherwise, go deeper
                        ...
                        let mut subtree = read_files(&path, depth + 1, min_bytes, regex, silent);
                        let dir_size = subtree.file_size;
                        if let Some(b) = min_bytes {
                            if dir_size >= FileSize::new(b) {
                                tree.push(path, dir_size, Some(&mut subtree), depth + 1);
